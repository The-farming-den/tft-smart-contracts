...
// SPDX-License-Identifier: MIT
// TFTToken with Governance.
contract TFTToken is BEP20('The Farmers Token', 'TFT') {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), 'TFT::delegateBySig: invalid signature');
        require(nonce == nonces[signatory]++, 'TFT::delegateBySig: invalid nonce');
        require(now <= expiry, 'TFT::delegateBySig: signature expired');
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, 'TFT::getPriorVotes: not yet determined');
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying TFTs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, 'TFT::_writeCheckpoint: block number exceeds 32 bits');
...
// SPDX-License-Identifier: MIT
// MasterChef is the master of Tft. He can make Tft and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once TFT is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TFTs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTftPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTftPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TFTs to distribute per block.
        uint256 lastRewardBlock; // Last block number that TFTs distribution occurs.
        uint256 accTftPerShare; // Accumulated TFTs per share, times 1e12. See below.
    ...
    // Update the given pool's TFT allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending TFTs on frontend.
    function pendingTft(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accTftPerShare = accTftPerShare.add(tftReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accTftPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        user.rewardDebt = user.amount.mul(pool.accTftPerShare).div(1e12);
